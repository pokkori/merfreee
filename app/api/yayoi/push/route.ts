...
import { pushJournal } from '@/lib/yayoi/client';
...
export async function POST() {
  ...
  if (!integration?.access_token) {
    return Response.json({ error: '弥生連携が設定されていません' }, { status: 400 });
  }
  ...
  for (const tx of transactions as Transaction[]) {
    try {
      const { journalId } = await pushJournal(
        accessToken,
      ...
      await supabase
        .from('transactions')
        .update({
          synced_to_yayoi_at: new Date().toISOString(),
          yayoi_journal_id: journalId,
        })
        .eq('id', tx.id);
